...
The query endpoint accepts an optional `queryLanguage` query parameter to select the query language. The default is `OPENCYPHER` (for the Neo4j backend). When the Fuseki backend is configured, use `queryLanguage=SPARQL` to issue SPARQL-star queries. Queries in a language not supported by the active backend are rejected with HTTP 501.
...
sequenceDiagram
    ...
    User ->> + API: POST /query?queryLanguage={OPENCYPHER|SPARQL}
