...
sequenceDiagram
    ...
    alt RDF content
        ...
        Verification -->> - API:OK
        API ->> + SDStore:storeSelfDescription
        SDStore ->> + GraphDB:addClaims
        GraphDB -->> - SDStore:OK
        SDStore -->> - API:OK
    else Non-RDF content
        API ->> + SDStore:storeAsset
        Note right of SDStore: No verification, no graph
        SDStore -->> - API:OK
    end
    API -->> - User:OK
....
...
Non-RDF assets (PDFs, plain JSON, YAML, images, binary files) are uploaded through the same `POST /self-descriptions` endpoint using `multipart/form-data` or `application/octet-stream`. The `RdfDetector` classifies the content; non-RDF assets bypass verification and graph storage entirely.
...
sequenceDiagram
    ...
    User ->> + API:POST /self-descriptions (multipart/form-data or application/octet-stream)
    ...
    Service ->> + SDStore:storeAsset(metadata, filename)
    SDStore ->> SDStore:generateSubjectId(prefix + hash)
    SDStore ->> SDStore:insertMetadata(PostgreSQL)
    SDStore ->> + FileStore:storeFile(hash, contentAccessor)
    FileStore -->> - SDStore:OK
    SDStore -->> - Service:storedMetadata
    Service -->> - API:SelfDescriptionMetadata
...
**Step 9:** `storeAsset` generates a content-addressable identifier (`urn:asset:sha256:<hash>`) if no subject ID is provided. If an asset with the same hash already exists, HTTP 409 (Conflict) is returned.
...
sequenceDiagram
    ...
    Note right of VerificationService: Filtered triples produce<br/>warnings in VerificationResult
    VerificationService -->> - API:VerificationResult (incl. warnings)
    API -->> -User: VerificationResult (incl. warnings)
....
...
==== Store Non-RDF Assets to the Self-Description store

This sequence diagram shows the flow when storing a non-RDF asset. Unlike Self-Descriptions, assets skip graph database interaction entirely. The content is stored in the asset File Store and metadata (including content type, file size, and original filename) is stored in the Metadata Store.
...
sequenceDiagram
    ...
    Service ->> + SDStore: storeAsset(metadata, filename)
    SDStore ->> SDStore: generateSubjectId(prefix + hash)
    SDStore ->> + MetaData: insertMetadata(hash, subjectId,<br>contentType, fileSize, filename)
    MetaData -->> - SDStore: OK
    SDStore ->> + FileStore: storeFile(hash, contentAccessor)
    FileStore -->> - SDStore: OK
    SDStore -->> - Service: storedMetadata
