...
The following examples show equivalent queries for the Fuseki backend using SPARQL-star syntax. These are sent via `POST /query?queryLanguage=SPARQL`.
...
Retrieve all Self-Description claims together with the credential subject they belong to, using RDF-star syntax:
...
Retrieve claims belonging to a specific Self-Description:
