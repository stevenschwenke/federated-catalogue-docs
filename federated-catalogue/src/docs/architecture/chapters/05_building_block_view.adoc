...
| File Store | The File Store is a blob storage. It holds Self-Description files, Schema files, and non-RDF assets. This includes historical versions of Self-Descriptions and Schemas. Non-RDF assets are stored in a dedicated File Store instance, separate from schema and context caches.
| Metadata Store | Store for metadata on Self-Descriptions, Schemas, and non-RDF assets stored in the File Store.
|===
...
==== File Store
The File Store is responsible to persist all file based content submitted to the catalogue. These are Self-Descriptions, Schemas, and non-RDF assets.
...
==== Metadata Store
The Metadata Store persists the metadata for the elements (Self-Descriptions, Schemas, Trust Anchors, and non-RDF assets). It allows to efficiently identify the relevant files in the file storage, to process the incoming requests.
...
| Self-Description Store | Storing and retrieving of Self-Description files and non-RDF assets, and handling their lifecycle and versioning. Self-Descriptions and assets are stored in the File Store, whereas metadata is kept in the Metadata Store. Non-RDF assets bypass verification and graph storage.
...
`CredentialVerificationStrategy` implements all verification and claim extraction logic. In `verifySelfDescription()`, it explicitly calls `ProtectedNamespaceFilter.filterClaims()` after claim extraction and propagates any filter warnings to the `VerificationResult`. The `VerificationStrategy` interface Javadoc documents the namespace filtering requirement (CAT-FR-GD-09) for all implementations.

[[_protected_namespace_filter]]
===== Protected Namespace Filter

The `ProtectedNamespaceFilter` prevents external actors from injecting triples that use the catalogue's reserved internal metadata namespace. Any such triples present in externally submitted self-descriptions or schemas are silently removed before storage or further processing.
...
| Self-Description upload and claim extraction | `CredentialVerificationStrategy.verifySelfDescription()` — explicitly calls `filterClaims()` after claim extraction; propagates filter warnings to `VerificationResult.warnings`.
...
| Graph rebuild (`POST /actuator/graph-rebuild`) | `GraphRebuilder.addSdToGraph()` — explicitly calls `filterClaims()` after re-extracting claims from stored self-descriptions, ensuring the rebuilt graph is clean.
|===
...
The Graph Store is the interface for interacting with the Graph Database. It receives claims (extracted from Self-Descriptions) and adds them to the graph database. The Graph Database only contains claims from active Self-Descriptions and offers a query interface (openCypher for the Neo4j backend, SPARQL-star for the Fuseki backend). External ontologies are not queried when processing requests. The interface also exposes the backend type and supported query language so that the service layer can validate incoming queries before they reach the database. Additionally, the interface provides health-check and claim-count methods used by the Spring Boot health indicator and the startup auto-rebuild mechanism.
...
Triggering a rebuild is **idempotent**. The rebuild re-imports claims from all active Self-Descriptions into the graph via `addClaims`, which overwrites any existing claims for the same credential subject. Running a rebuild on an already up-to-date graph produces the same end state. If a rebuild is already in progress, the endpoint returns HTTP 409 (Conflict) and no second rebuild is started.
...
The Self-Description Store is responsible for storing Self-Description files (whole files, not individual VCs), non-RDF assets, and their metadata.

For **RDF Self-Descriptions**, the store first saves the metadata in the Metadata Store, then adds claims to the Graph database, and finally stores the file in the File Store. A transaction ensures that the stored data is consistent.
...
public interface SelfDescriptionStore {
   ...
   *
   * @param sdMetadata The asset metadata (hash, content type, file size, content accessor).
...
The catalogue supports uploading non-RDF content (PDFs, plain JSON, YAML, images, binary files) through the same `POST /self-descriptions` endpoint, extended to accept `multipart/form-data` and `application/octet-stream` content types.
...
....
flowchart TD
    Upload[POST /self-descriptions] --> Extract[Extract bytes + content type]
    Extract --> Detect{RdfDetector:\nis RDF?}
    Detect -->|Yes| Verify[Verification Pipeline]
    Verify --> StoreCred[storeSelfDescription\nmetadata + claims + graph]
    Detect -->|No| StoreAsset[storeAsset\nmetadata + FileStore only]
...
* `ContentAccessorDirect` — wraps UTF-8 text content (used for RDF Self-Descriptions and Schemas).
