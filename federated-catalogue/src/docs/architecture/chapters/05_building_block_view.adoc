...
| Verification
a| Responsible for applying the defined verification rules on credentials (Verifiable Presentations or standalone Verifiable Credentials) and extracting claims. Accepts JWT-wrapped credentials per ICAM v24.07 (Gaia-X Loire) and generic VC 2.0 JSON-LD.
...
* **Format detection:** `FormatDetector` classifies incoming credentials into two verification paths: Loire (JWT per ICAM v24.07) or generic VC 2.0. Credentials without a VC 2.0 context URI are rejected as `UNKNOWN`. See <<_format_detection>>.
...
The `FormatDetector` classifies incoming credentials into one of the following verification paths based on JWT header values (`typ`, `cty`) and payload structure. All credentials must include the W3C VC 2.0 context URI (`\https://www.w3.org/ns/credentials/v2`). Credentials using the VC 1.1 context (`\https://www.w3.org/2018/credentials/v1`) or Linked Data proofs are rejected as `UNKNOWN`.
...
| Unrecognized | `UNKNOWN` | No VC 2.0 context, or VC 1.1 context, or Linked Data proof → rejected
|===
