...
* **Format detection:** `FormatDetector` classifies incoming RDF content. Recognized credential formats (Loire JWT, VC 2.0) enter the credential verification pipeline. Content classified as `UNKNOWN` (no recognized credential format) is routed to the non-credential RDF extraction path — all triples are extracted via `JenaAllTriplesExtractor`. See <<_format_detection>>.
* **Non-credential RDF extraction:** For RDF assets that are not Gaia-X Credentials, `JenaAllTriplesExtractor` extracts all triples using Apache Jena. Supports JSON-LD, Turtle, N-Triples, and RDF/XML via a format fallback sequence. Returns a `NonCredentialVerificationResult` containing all extracted `RdfClaim` instances. See <<_non_credential_rdf_extraction>>.
...
classDiagram
    class VerificationService {
        ...
        +extractClaims(payload) List~RdfClaim~
...
classDiagram
    ...
    class JenaAllTriplesExtractor {
        +extractAllTriples(payload) List~RdfClaim~
    }
    ...
    CredentialVerificationStrategy --> JenaAllTriplesExtractor : extracts all triples\n(UNKNOWN format)
    CredentialVerificationResult <|-- NonCredentialVerificationResult
....

`VerificationServiceImpl` injects the strategy via `@Qualifier("credentialVerificationStrategy")`, providing compile-time safety.
The `resolveStrategy(ContentAccessor)` method always resolves to `CredentialVerificationStrategy`, which internally routes between credential verification and non-credential RDF extraction based on `FormatDetector` output.

`CredentialVerificationStrategy` implements all verification and claim extraction logic.
When `FormatDetector` returns a recognized credential format (`GAIAX_V2_LOIRE` or `VC2_DANUBETECH`), the full credential verification pipeline runs.
When it returns `UNKNOWN`, the strategy delegates to `JenaAllTriplesExtractor` to extract all triples from the raw RDF content — returning a `NonCredentialVerificationResult`.
...
For RDF assets that are not verifiable credentials , `CredentialVerificationStrategy` short-circuits the credential verification pipeline when `FormatDetector` returns `UNKNOWN` and delegates to `JenaAllTriplesExtractor` instead.

`JenaAllTriplesExtractor` uses Apache Jena to parse the raw RDF content and extract all triples.
It tries each RDF serialization in order until one succeeds: JSON-LD → Turtle → N-Triples → RDF/XML.
All extracted triples are returned as `List<RdfClaim>`.
Blank nodes are handled via Jena's built-in blank node representation.

The extraction returns a `NonCredentialVerificationResult` — a subclass of `CredentialVerificationResult` with `issuer`, `issuanceDate`, and `credentialId` set to `null` (absent from non-credential RDF documents).
The `claims` field contains all extracted `RdfClaim` instances, which pass through `ProtectedNamespaceFilter` before being stored in the graph.
...
| Generic VC 2.0 (JSON-LD or JWT with `vc`/`vp` wrapper) | `VC2_DANUBETECH` | VC 2.0 context present; does not match Loire header criteria
| Non-credential RDF | `UNKNOWN` | No recognized credential format — all triples extracted via `JenaAllTriplesExtractor`
|===
...
flowchart TD
    ...
    Format -->|UNKNOWN| ExtractAll[JenaAllTriplesExtractor:\nextract all triples]
