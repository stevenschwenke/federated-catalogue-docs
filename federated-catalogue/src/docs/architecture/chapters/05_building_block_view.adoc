...
The Federated Catalogue offers the possibility to verify credentials (Verifiable Presentations / Verifiable Credentials). Two credential formats are supported: JSON-LD with data-integrity proofs (Gaia-X Tagus) and JWT-wrapped credentials per ICAM v24.07 (Gaia-X Loire). The verification pipeline consists of four independently configurable stages: syntactic parsing, semantic validation, schema validation (SHACL), and cryptographic signature verification. Each stage can be enabled or disabled via configuration properties. A `FormatDetector` classifies incoming credentials and routes them to the appropriate verification path (see ADR 8).
...
| Verification
a| Responsible for applying the defined verification rules on credentials (Verifiable Presentations or standalone Verifiable Credentials) and extracting claims. Supports both JSON-LD credentials with data-integrity proofs (Tagus format) and JWT-wrapped credentials per ICAM v24.07 (Loire format).
...
* **Format detection:** `FormatDetector` classifies incoming credentials into three verification paths: Path 1 (Tagus JSON-LD with data-integrity proofs), Path 2 (Loire JWT per ICAM v24.07), or Path 3 (non-Gaia-X). See <<_format_detection>>.
...
* **Claim Extractor:** Extracts claims from Verifiable Presentations and standalone Verifiable Credentials. Supports both VC 1.1 (`issuanceDate`/`expirationDate`) and VC 2.0 (`validFrom`/`validUntil`) date fields.
...
classDiagram
    ...
    class FormatDetector {
        +detect(headers, payload) CredentialFormat
    }
    class JwtContentPreprocessor {
        +isJwtWrapped(contentType, body) boolean
        +unwrap(body) JsonLDObject
    ...
    class LoireJwtParser {
        +parse(compactJwt) JsonLDObject
    }
    ...
    CredentialVerificationStrategy --> FormatDetector : classifies format
    CredentialVerificationStrategy --> JwtSignatureVerifier : verifies JWT signatures
    CredentialVerificationStrategy --> JwtContentPreprocessor : detects + unwraps JWT
    CredentialVerificationStrategy --> LoireJwtParser : parses Loire JWT payload
....
...
The `FormatDetector` classifies incoming credentials into one of three verification paths based on JWT header values (`typ`, `cty`) and payload structure:

[options="header",cols="1,2,3"]
|===
| Path | Format | Detection criteria
| Path 1 (Tagus) | JSON-LD with data-integrity proof | No JWT envelope; JSON-LD body with `proof` field
| Path 2 (Loire) | JWT per ICAM v24.07 / W3C VC-JOSE-COSE | JWT envelope with recognized `typ`/`cty` headers (see table below)
| Path 3 (non-Gaia-X) | Other JSON-LD or JWT formats | Does not match Path 1 or Path 2 criteria
|===
...
The `JwtSignatureVerifier` is a Spring `@Component` that performs cryptographic verification of compact JWT serializations. It is invoked by `CredentialVerificationStrategy` when signature verification is enabled (`verifyVCSignatures=true` or `verifyVPSignatures=true`).
...
**Inner VC verification in VP JWTs:** When `verifyVCSignatures=true` and the VP JWT contains `EnvelopedVerifiableCredential` entries (ICAM v24.07 format), each inner VC JWT is extracted from its `data:application/vc+ld+json+jwt,{jwt}` URL and independently verified via `JwtSignatureVerifier`. Mixed VP content (some enveloped JWTs, some inline JSON-LD VCs with `proof` fields) is supported — each inner VC is verified by its respective path.
...
The JWT is extracted from the `data:` URL by stripping the media type prefix. The parser also supports VC 2.0 credentials that use the `https://www.w3.org/ns/credentials/v2` context URL and `validFrom`/`validUntil` date fields instead of the VC 1.1 `issuanceDate`/`expirationDate`.
...
Both credential formats are supported in the strict profile:

* **Tagus (JSON-LD + data-integrity proof):** Signature verified via the existing LD proof path (`UniSignatureVerifier`).
* **Loire (JWT per ICAM v24.07):** Signature verified via `JwtSignatureVerifier` using DID document key resolution (see <<_jwt_signature_verification>>). VP JWT inner VCs (`EnvelopedVerifiableCredential`) are also verified.

[options="header",cols="1,1"]
|===
| Stage | Status
| Syntactic parsing | Always active
| Format detection | Always active (classifies Tagus vs. Loire vs. non-Gaia-X)
| Semantic validation | *Active*
| Schema validation (SHACL) | *Active*
| Signature verification (LD proof + JWT) | *Active*
| Trust framework (Gaia-X) | *Active*
|===

**Capabilities:** Full Gaia-X compliance enforcement. JSON-LD credentials must carry a valid data-integrity proof. JWT credentials must be signed with a key resolvable from the issuer's DID document. In both cases, the DID document must contain an `x5u` URL pointing to a valid certificate chain registered in the Trust Anchor Registry. Only `did:web` identifiers are accepted when the trust framework is enabled.
...
flowchart LR
    subgraph strict ["Strict Profile"]
        ...
        E --> F{JWT or LD proof?}
        F --> |LD proof| G1(Verify LD proof\nsignatures)
        F --> |JWT| G2(Verify JWT signature\nvia DID document)
        G1 --> H(Resolve DID +\ncheck public key)
        G2 --> H
...
flowchart TB
    ...
    subgraph pipeline ["Verification Pipeline"]
        ...
        SIG --> |yes| SIGFMT{JWT or LD proof?}
        SIG --> |no| EXT
        SIGFMT --> |LD proof| LDVP(Verify LD proof\nsignatures)
        SIGFMT --> |JWT| JWTVP(Verify JWT signature\nvia DID document)
        LDVP --> TF{Gaia-X Trust Framework\nenabled?}
        JWTVP --> TF
    ...
    style LDVP fill:#fff3e0,stroke:#ff9800
