...
For **non-RDF assets**, the store skips verification and graph interaction entirely. It stores metadata in the Metadata Store (with `content_type`, `file_size`, and `original_filename`), writes the raw bytes to the asset File Store, and auto-generates a content-addressable identifier (`urn:asset:sha256:<hash>`).
...
public interface AssetStore {
   ...
   *
   * @param hash The hash value that identifies the asset.
   ...
   * @param withMeta flag indicating the full metaData should be loaded instead of just the hash.
   * @param withContent flag indicating the content should also be returned.
   * @return List of all asset meta-data objects that match the specified filter.
   */
  PaginatedResults<AssetMetadata> getByFilter(AssetFilter filter, boolean withMeta, boolean withContent);

  /**
   * Store the given credential (Verifiable Presentation or standalone Verifiable Credential).
   *
   * @param credential            The credential metadata to store.
   * @param verificationResults   The results of the verification of the credential.
   */
  ...
  /**
   * Store a non-RDF asset. Bypasses verification and graph storage.
   * Writes metadata to the Metadata Store and content to the asset File Store.
   *
   * @param assetMetadata The asset metadata (hash, content type, file size, content accessor).
   * @param originalFilename The original filename from the upload (may be null).
   * @return The stored metadata with auto-generated subject ID.
   * @throws ConflictException if an asset with the same hash already exists.
   */
...
/**
 * Filter parameters for searching asset meta data. If multiple items
...
NOTE: The `content` column is nullable. For credentials (RDF), it stores the JSON-LD in the database. For non-RDF assets, content is stored only in the File Store and the database column remains `NULL`. The three asset metadata columns (`content_type`, `file_size`, `original_filename`) are backfilled with `content_type = 'application/ld+json'` for existing records. The table was renamed from `sdfiles` to `assets` and the primary key from `sdhash` to `asset_hash` (see Liquibase changeset `2026-03-09-rename-sdfiles-to-assets`).
...
The `AssetUploadService` orchestrates the dual-path upload flow:
...
flowchart TD
    ...
    Verify --> StoreCred[storeCredential\nmetadata + claims + graph]
    Detect -->|No| StoreAsset[storeUnverified\nmetadata + FileStore only]
...
* Auto-generated ID: `urn:asset:sha256:<content-hash>`
* Duplicate content rejected with HTTP 409 (Conflict)
...
| `federated-catalogue.asset.subject-id-prefix` | `urn:asset:sha256:` | Prefix for auto-generated non-RDF asset identifiers
